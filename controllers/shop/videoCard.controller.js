...
module.exports = {
  enter: async (ctx) => {
    ...
    const list = getVideoCards();
    await ctx.reply(
      messages.shopDetailsListMessage({ list, currentId: player.videocardId }),
  ...
  information: async (ctx) => {
    ...
    await ctx.replyWithMarkdown(
      messages.shopVideoCardInfoMessage(),
  ...
  buy: async (ctx) => {
    ...
    const videoCard = getVideoCardObjectById(idToBuy);

    if (player.virtualMoney < videoCard.price) {
